...
## 5.1 Local service samples

Let's start a sample that creates and calls a service!

- On Windows start:
  - `C:\eCAL\samples\bin\ecal_sample_minimal_server_c.exe`
  - and `C:\eCAL\samples\bin\ecal_sample_minimal_client_c.exe`

- On Ubuntu start:
  - `ecal_sample_minimal_server_c`
  - and `ecal_sample_minimal_client_c`

![eCAL Service communication](../../../assets/samples_service.png)

<Aside>Right after you started the client, it may print a few errors about the service not being found. Available eCAL services are propagated every 500ms, so it will always take a short amount of time until a service is known to exist.</Aside>
...
<Aside type="tip">
To easily edit the hosts file on **Windows**, press `Win` + `R`, enter
   
   ``notepad.exe C:\Windows\System32\drivers\etc\hosts``
   
   and then press `Ctrl` + `Shift` + `Enter` to open the file as administrator.
</Aside>

## 5.3 Service Sample over network

Now start one `ecal_sample_minimal_server_c` and one `ecal_sample_minimal_client_c`, just as above.
...
![eCAL service samples over network](../../../assets/samples_service_cloud_combined.png)